...
def get_recommendations(disease_name, top_n=5, min_score=0):
...
for disease in diseases_df["disease_name"].tolist():
...
if st.button("Analyze"):
    result = get_recommendations(selected_disease, top_n=top_n, min_score=min_score)

    if result.empty:
        st.warning("No recommendations found for the selected score threshold.")
    else:
